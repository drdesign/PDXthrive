...
add_action( 'admin_init', 'woga_activate_license' );
add_action( 'admin_init', 'woga_register_settings' );

function woga_register_settings()
{
...
function woga_license_view(){

    $status =  woga_get_status();
    $value = woga_get_license();
    echo '<p ><input '.(($status === 'valid') ? ('style="border:1px solid #7ad03a; "') : '').' type="text" id="woga_license" name="woga_license" value="' . $value . '" /><a '.(($status === 'valid') ? ('style="color:#fff;background:  #7ad03a; display: inline-block;text-decoration: none;font-size: 13px;line-height: 26px;height: 26px; margin-left:5px; padding: 0 10px 1px;  -webkit-border-radius: 3px;border-radius: 3px; ">Valid') : ('style="color:#fff;background:  #dd3d36; display: inline-block;text-decoration: none;font-size: 13px;line-height: 26px;height: 26px; margin-left:5px; padding: 0 10px 1px;  -webkit-border-radius: 3px;border-radius: 3px; ">Invalid')).' </a></p><p class="description">Enter your license from <a  href="https://themeisle.com/purchase-history">themeisle.com</a> in order to get theme updates</p>';


}

function woga_get_status(){

    $license_data = get_option( 'woga_license_data', '' );

    if($license_data !== ''){
        return isset($license_data->license) ? $license_data->license : get_option( 'woga_license_status','' ) ;
    }else{
        return get_option( 'woga_license_status','' ) ;
    }
}

function woga_get_license(){

    $license_data = get_option( 'woga_license_data', '' );
    if($license_data !== ''){
        return isset($license_data->key) ? $license_data->key: get_option( 'woga_license', '' ) ;
    }else{
        return get_option( 'woga_license','' ) ;
    }
}
function woga_check_activation(){
    $license_data = get_option( 'woga_license_data', '' );
...
function woga_check_expiration(){
...
function woga_check_hide($hide){
    if(isset($_GET['woga_hide_'.$hide])){
        if($_GET['woga_hide_'.$hide]==='yes') {
            $license = get_option( 'woga_license_data', '' );
            $license->{'hide_'.$hide} = true;
            update_option( 'woga_license_data', $license );
            return false;
        }
    }else{
        $license =
...
function woga_notice() {
    ...
    if($status != 'valid')  {
        ...
        ?>
        <?php if(woga_check_hide('valid') ): ?>
            <div class="error">
                <p><strong>You do not have a valid license for <?php echo get_current_theme(); ?>  theme !!!. You can get the license code from your purchase history on <a href="https://themeisle.com/purchase-history" >themeisle.com</a> and validate it <a href="<?php echo admin_url("options-general.php"); ?>#woga_license">here</a> </strong>| <a href="<?php echo $admin_url; ?>?woga_hide_valid=yes">Hide Notice</a></p>
            </div>
        <?php endif; ?>
    <?php
    }else{

        if(woga_check_expiration()){
            if(woga_check_hide('expiration')){
                ?>
                ?>
                <div class="update-nag">
                    <p><strong>Your license is about to expire for <?php echo get_current_theme(); ?>   theme !!!. You can go to  <a href="https://themeisle.com/" >themeisle.com</a> and  renew it.</strong>| <a href="<?php echo $admin_url; ?>?woga_hide_expiration=yes">Hide Notice</a></p>
                </div>
            <?php
            }
        }
...
add_action( 'admin_notices', 'woga_notice' );
function woga_renew_url(){
...
function woga_activate_license() {

    // listen for our activate button to be clicked
    if( isset( $_POST['woga_license'] ) ) {


        // retrieve the license from the database
        $license = $_POST['woga_license'];
        ...
        update_option( 'woga_license_data', $license_data );
        delete_transient( 'woga_license_data');
        set_transient( 'woga_license_data', $license_data, 12 * HOUR_IN_SECONDS  );

    }
}

add_action( 'admin_init', 'woga_theme_valid',9999999 );
function woga_theme_valid($force = false){
    if ( false === ( $license = get_transient( 'woga_license_data' ) ) ) {
        $license = woga_check_license();
        set_transient( 'woga_license_data', $license, 12 * HOUR_IN_SECONDS   );
        update_option( 'woga_license_data', $license );
    }

}

function woga_check_license() {

    global $wp_version;


    $theme_data =   wp_get_theme(basename(get_template_directory() ));
    $license = trim( woga_get_license() );
    $api_params = array(
    ...
    $license_old = get_option( 'woga_license_data', '' );
...
function woga_theme_updater() {


    $theme_data =   wp_get_theme(basename(get_template_directory() ));
    $test_license = trim( woga_get_license() );
...
add_action( 'admin_init', 'woga_theme_updater' );
